...
# df = api_request(api_endpoint=parking_meter_occupancy_api_endpoint, api_name='Socrata')
...
# df = pd.merge(df, inventory_df, how='inner', left_on='spaceid', right_on='space_id')

# # df = pd.read_sql_query(q, conn)
# lat = [float(i) for i in df['lat']]
# long = [float(i) for i in df['long']]
# df['lat'] = lat
# df['long'] = long
# st.map(df[['lat', 'long']].rename(columns={'lat': 'lat', 'long': 'lon'}))
# st.write(df.shape)
# st.write(len(df['space_id'].unique()))
